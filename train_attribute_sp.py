...
if __name__ == "__main__":
    ...
    # scheduler_w = optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer_waveform, mode='min', factor=0.8, patience=1)
    # scheduler_s = optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer_spectrogram, mode='min', factor=0.8, patience=1)
    # scheduler_c = optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer_compressor, mode='min', factor=0.8, patience=1)
    
    scheduler_w = optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer_waveform, T_max=50, eta_min=1e-6)
    scheduler_s = optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer_spectrogram, T_max=50, eta_min=1e-6)
    scheduler_c = optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer_compressor, T_max=50, eta_min=1e-6)
    
    model_waveform.load_state_dict(torch.load('cat_emb/result/tas_5/best_model_w.pt'))
    model_spectrogram.load_state_dict(torch.load('cat_emb/result/tas_5/best_model_s.pt'))
    emb_comp.load_state_dict(torch.load('cat_emb/result/tas_5/best_model_c.pt'))
