...
best_model_path_waveform_machine = 'cat_emb/result/tms_2/best_model_w.pt'
best_model_path_spectrogram_machine = 'cat_emb/result/tms_2/best_model_s.pt'
best_model_path_compressor_machine = 'cat_emb/result/tms_2/best_model_c.pt'
...
model_waveform.load_state_dict(torch.load(best_model_path_waveform_machine))
model_spectrogram.load_state_dict(torch.load(best_model_path_spectrogram_machine))
emb_comp.load_state_dict(torch.load(best_model_path_compressor_machine))
...
with open('cat_emb/result_emb/train_machine_emb.pickle', 'wb') as f:
...
with torch.no_grad():
    ...
    for batch in tqdm(test_loader):
        ...
        result_test.append(feature_compressed.cpu().numpy())

result_test = np.array(result_test).reshape(-1, 512)
with open('cat_emb/result_emb/test_machine_emb.pickle', 'wb') as f:
...
with torch.no_grad():
    ...
    for batch in tqdm(eval_loader):
        ...
        result_eval.append(feature_compressed.cpu().numpy())

result_eval = np.array(result_eval).reshape(-1, 512)
with open('cat_emb/result_emb/eval_machine_emb.pickle', 'wb') as f:
